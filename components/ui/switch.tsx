...
const switchVariants = cva(
  "peer relative  inline-flex items-center  [&_.content-box>svg]:h-4  [&_.content-box>svg]:w-4  [&_.content-box]:text-primary-foreground [&_.content-box]:text-[10px] justify-start group  shrink-0  cursor-pointer rounded-full  transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50 data-[state=unchecked]:bg-default-300",
  {
    variants: {
      color: {
...
function Switch({
  ...
  thumbIcon,
  ...props
}: React.ComponentProps<typeof SwitchPrimitives.Root> & SwitchProps) {
  return (
    ...
    >
      ...
      <SwitchPrimitives.Thumb
        className={cn(
          "pointer-events-none z-10 origin-right  h-4 w-4 rounded-full bg-background  shadow-lg ring-0 transition-all  flex items-center justify-center   data-[state=checked]:ltr:ml-5 data-[state=checked]:rtl:mr-5 data-[state=unchecked]:rtl:mr-0.5 ",
          thumbClass,
          {
