...
const sliderVariants = cva(
  "relative flex w-full touch-none select-none  items-center   data-disabled:opacity-50 data-[orientation=vertical]:w-2 data-[orientation=vertical]:h-full  data-[orientation=vertical]:flex-col   ",
  {
    variants: {
      color: {
        primary:
          "  [&_.range-thumb]:border-secondary [&_.range-thumb]:drop-shadow-xs [&_.range-thumb]:bg-primary  [&_.range-thumb]:focus-visible:bg-primary",
        info: "[&_.range]:bg-info [&_.range-thumb]:border-secondary [&_.range-thumb]:drop-shadow-xs [&_.range-thumb]:bg-info  [&_.range-thumb]:focus-visible:bg-info",
        warning:
          "[&_.range]:bg-warning [&_.range-thumb]:border-secondary [&_.range-thumb]:drop-shadow-xs [&_.range-thumb]:bg-warning  [&_.range-thumb]:focus-visible:bg-warning",
        success:
          "[&_.range]:bg-success [&_.range-thumb]:border-secondary [&_.range-thumb]:drop-shadow-xs [&_.range-thumb]:bg-success  [&_.range-thumb]:focus-visible:bg-success",
        destructive:
          "[&_.range]:bg-destructive [&_.range-thumb]:border-secondary [&_.range-thumb]:drop-shadow-xs [&_.range-thumb]:bg-destructive  [&_.range-thumb]:focus-visible:bg-destructive",
        dark: "[&_.range]:bg-default-950 [&_.range-thumb]:border-secondary [&_.range-thumb]:drop-shadow-xs [&_.range-thumb]:bg-default-950  [&_.range-thumb]:focus-visible:bg-default-950",
      },
          ...
          return (
            <React.Fragment key={`thumb-${index}`}>
              {showTooltip ? (
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <SliderPrimitive.Thumb
                        key={`thumb-${index}`}
                        className="block h-5 w-5  cursor-pointer rounded-full border-2 range-thumb ring-offset-background transition-colors focus-visible:outline-hidden   disabled:pointer-events-none disabled:opacity-50 "
                      />
                    </TooltipTrigger>
                ...
                <SliderPrimitive.Thumb
                  key={`thumb22-${index}`}
                  className="block h-5 w-5 cursor-pointer rounded-full ring-2 ring-background range-thumb  focus-visible:ring-4 ring-offset-background transition-colors focus-visible:outline-hidden   disabled:pointer-events-none disabled:opacity-50 "
                />
              )}
            </React.Fragment>
          );
